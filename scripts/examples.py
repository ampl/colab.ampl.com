...
def config_book_example(
    ...
    if md_fname:
        markdown_header = open(f"{AMPL_BOOK_SRC_DIR}/{md_fname}", "r").read()
    else:
        markdown_header = f"### Example: {fname}\n"
        markdown_header += (
            f"autogenerated using {mod_fname}, {dat_fname}, and {run_fname}"
        )
        # open(f"{AMPL_BOOK_SRC_DIR}/{fname}.md", "w").write(markdown_header)
    return {
        "name": fname,
        ...
        "ampl_commands": open(f"{AMPL_BOOK_SRC_DIR}/{run_fname}", "r").read(),
    }
...
def config_ampl_lecture(fname, md_fname, title=None, description=None):
    return {
        "name": fname,
        "markdown": open(f"{AMPL_LECTURE_SRC_DIR}/{md_fname}", "r").read(),
        "title": title,
        "description": description,
